...
const createMenuButtons = (input, handleClick) => {
    return (
        <>
        <button className={`flex p-0 w-full h-20 text-black justify-center items-center`}key={input.id} onClick={handleClick} >
            {input.tag}
        </button>
        </>
    );
...
const createMenu = (values, handleClick, showUserMenu, setShowUserMenu, width) => {
    ...
    let menuPosition = width <= 1024
     ...
     return (
        ...
        <div onClick={() => { setShowUserMenu(false); }} className="absolute top-[6.25rem] left-0 h-full w-full"></div>
            ...
            {values.map((input) => (
                createMenuButtons( input, handleClick)
            ))}
        </div>
        </>
     );
...
export default function Headers(props){
    ...
    <div className={`flex h-24 ${background} w-full sm:px-8 md:px-12 lg:px-24`}>
        <div className="flex items-center w-6/12 pl-4 text-white">
            ...
            {createMenu(props.values, handleClick, showUserMenu, setShowUserMenu, width)}
        </div>
