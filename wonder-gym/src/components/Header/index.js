...
const createMenu = (values, showUserMenu, setShowUserMenu, width) => {
    ...
    let menuPosition = width <= 1024
        ? `${commonProps} left-0 ${showUserMenu ? 'translate-x-0':'-translate-x-full hiden'} ease-in-out duration-300`
        : `${commonProps} right-0 ${showUserMenu ? 'translate-x-0':'translate-x-full hiden'} ease-in-out duration-300`;
...
export default function Headers(props){
    ...
    <div className={`flex h-24 ${background} w-full px-8 md:px-12 lg:px-24`}>
        <div className="flex items-center w-6/12 text-white">
            ...
            {showUserMenu && (<div onClick={() => { setShowUserMenu(false); }} className="absolute top-[6.25rem] left-0 h-full w-full"></div>)}
